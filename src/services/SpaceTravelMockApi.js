...
class SpaceTravelMockApi {
  ...
  static async getSpacecraftById({ id }) {
    ...
    try {
      ...
      for (const spacecraft of mockDb.spacecrafts) {
        if (spacecraft.id === id) {
  ...
  static async destroySpacecraftById({ id }) {
    ...
    try {
      ...
      const index = mockDb.spacecrafts.findIndex(
        (spacecraft) => spacecraft.id === id,
      );

      if (index !== -1) {
        mockDb.spacecrafts.splice(index, 1);
        SpaceTravelMockApi.setMockDb(mockDb);
      }
    } catch (error) {
      response.isError = true;
  ...
  static async sendSpacecraftToPlanet({ spacecraftId, targetPlanetId }) {
    ...
    try {
      ...
      const destinationId = Number(targetPlanetId);

      const spacecraft = mockDb.spacecrafts.find(
      ...
      const currentLocationId = Number(spacecraft.currentLocation);

      if (currentLocationId === destinationId) {
        throw new Error("The spacecraft is already on this planet!");
      }

      const currentPlanet = mockDb.planets.find(
        (planet) => Number(planet.id) === currentLocationId,
      );

      const targetPlanet = mockDb.planets.find(
        (planet) => Number(planet.id) === destinationId,
      );
      ...
      const passengersToMove = Math.min(
        Number(spacecraft.capacity),
        Number(currentPlanet.currentPopulation),
      );

      currentPlanet.currentPopulation -= passengersToMove;
      targetPlanet.currentPopulation += passengersToMove;
      spacecraft.currentLocation = destinationId;
